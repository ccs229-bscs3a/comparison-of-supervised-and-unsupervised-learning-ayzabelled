...
def app():
    ...
    st.subheader('K-means clustering applied to the Oxford Flowers Dataset')
    text = """This is a classic example of unsupervised learning. The Oxford Flowers dataset contains images of various flower species. K-means clustering doesn't use pre-existing labels (flower species) during the grouping process.

    * **K-means Clustering:** The algorithm aims to group data points into a predefined number of clusters (k). It iteratively assigns each data point to the nearest cluster centroid (center) and recomputes the centroids based on the assigned points. This process minimizes the within-cluster distances, creating groups with similar characteristics.
    In essence, K-means helps uncover inherent groupings within the flower images based on their features extracted from the images (color, texture, etc.), without relying on predefined categories (flower species). This allows us to explore how well the data separates into natural clusters, potentially corresponding to the actual flower species.

    * Choosing the optimal number of clusters (k) is crucial. The "elbow method" helps visualize the trade-off between increasing clusters and decreasing improvement in within-cluster distances.
    * K-means is sensitive to initial centroid placement. Running the algorithm multiple times with different initializations can help identify more stable clusters.

    By applying K-means to the Oxford Flowers dataset, we gain insights into the underlying structure of the flower images and potentially validate the separability of the different flower species based on their visual characteristics."""
    ...
    if st.button("Begin"):
        ...
        data = pd.read_csv("path/to/oxford_flowers.csv")
        ...
        * The Oxford Flowers dataset is relatively well-separated into three flower species. A Silhouette Score above 0.5 might be achievable with an appropriate number of clusters (k=3) and good initialization.
        * The optimal k can vary depending on the specific dataset and the desired level of granularity in the clustering."""
        ...
        for label, color in zip(unique_labels, colors):
            ...
            ax.scatter(X[indices, 0], X[indices, 1], label=data.target_names[label], c=color)

        # Add labels and title using ax methods
        ax.set_xlabel('Sepal length (cm)')
        ax.set_ylabel('Sepal width (cm)')
        ax.set_title('Sepal Length vs Width Colored by Predicted Oxford Flowers Species')
